...
module response_collector_top (
  ...
  response_collector #(
    ...
    .clk_i(clk)
    ,.rst_n_i(rst_n)
    ,.rx_fetch_resp_valid_i(rx_valid_i)
    ,.rx_fetch_resp_ready_o(rx_ready_o)
    ,.rx_fetch_resp_is_factor_i(rx_is_factor_i)
    ,.rx_fetch_resp_state_words_i(rx_state_words_i)
    ,.rx_fetch_resp_data_i(rx_data_i)
    ,.rx_fetch_resp_data_valid_i(rx_data_valid_i)
    ,.rx_fetch_resp_last_i(rx_last_i)
    ,.rx_fetch_resp_done_valid_i(rx_done_valid_i)
    ,.rx_fetch_resp_txn_id_i(rx_txn_id_i)
    ,.rx_fetch_resp_node_id_i(rx_node_id_i)
    ,.rx_fetch_resp_consumer_node_id_i(rx_consumer_node_id_i)
