...
module multi_node_concurrent_top (
  ...
  pull_client u_pull_client (
    ...
    ,.tx_fetch_req_valid_o(fetch_req_valid)
    ,.tx_fetch_req_ready_i(fetch_req_ready)
    ,.tx_fetch_req_target_node_id_o(fetch_req_target_node_id)
    ,.tx_fetch_req_consumer_node_id_o(fetch_req_consumer_node_id)
    ,.tx_fetch_req_is_factor_o(fetch_req_is_factor)
    ,.tx_fetch_req_target_x_o(fetch_req_target_x)
    ,.tx_fetch_req_target_y_o(fetch_req_target_y)
    ,.tx_fetch_req_txn_id_o(fetch_req_txn_id)
    ,.tx_fetch_req_store_idx_o()
  );

  response_collector u_response_collector (
    .clk_i(clk)
    ,.rst_n_i(rst_n)
    ,.rx_fetch_resp_valid_i(rx_resp_valid)
    ,.rx_fetch_resp_ready_o(rx_resp_ready)
    ,.rx_fetch_resp_is_factor_i(rx_resp_is_factor)
    ,.rx_fetch_resp_state_words_i(rx_resp_state_words)
    ,.rx_fetch_resp_data_i(rx_resp_data)
    ,.rx_fetch_resp_data_valid_i(rx_resp_data_valid)
    ,.rx_fetch_resp_last_i(rx_resp_last)
    ,.rx_fetch_resp_done_valid_i(rx_resp_done_valid)
    ,.rx_fetch_resp_txn_id_i(rx_resp_txn_id)
    ,.rx_fetch_resp_node_id_i(rx_resp_node_id)
    ,.rx_fetch_resp_consumer_node_id_i(rx_resp_consumer_node_id)
    ,.staging_wr_valid_o()
