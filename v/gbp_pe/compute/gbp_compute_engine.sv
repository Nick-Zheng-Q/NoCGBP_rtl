...
#(
  ...
  function automatic logic [15:0] compact_payload_beats(input logic [2:0] dofs);
    int words;
    begin
  ...
  function automatic logic [15:0] bytes_to_beats(input logic [15:0] xfer_bytes);
    begin
  ...
  function automatic logic [15:0] accumulate_message_beats(
      input logic [3:0] msg_count, input logic [MAX_ADJACENT-1:0][2:0] neighbor_dofs);
    logic [15:0] total;
    int unsigned idx;
    begin
      total = 16'd0;
  ...
  // Stream direction derived directly from FSM state (no register delay)
  logic         is_output_dir;
  assign is_output_dir = (u_gbp_control_fsm.state_r == GFSM_VAR_STORE_RESULT)
                       || (u_gbp_control_fsm.state_r == GFSM_FAC_STORE_MESSAGE);
  ...
  assign buf_stream_rd_valid = gbp_internal_rd_valid
                               || ((stream_active_r && is_output_dir) || start_output_stream);
  ...
  always_ff @(posedge clk_i) begin
    if (stream_in_hs)
      $display(
          "ENG_HS: beats=%0d target=%0d fsm=%0d xfer_bytes=%0d",
          stream_in_beats_r,
          stream_target_beats,
          u_gbp_control_fsm.state_r,
          gbp_stream_xfer_bytes
      );
    if (gbp_stream_done)
      $display(
          "ENG_DONE: dir_out=%0d in_hs=%0d beats=%0d target=%0d",
          is_output_dir,
          stream_in_hs,
          stream_in_beats_r,
          stream_target_beats
  ...
  assign stream_target_beats = is_output_dir ? bytes_to_beats(
      cmd_wr_xfer_bytes_r
  ) : (cmd_is_factor_r ? state_target_beats : ((gbp_stream_req_state && gbp_stream_req_messages) ?
                                               (state_target_beats + accumulate_message_beats(
      cmd_msg_count_r, cmd_neighbor_dofs_r
  )) : (gbp_stream_req_state ?
        state_target_beats : (gbp_stream_req_messages ? accumulate_message_beats(
      cmd_msg_count_r, cmd_neighbor_dofs_r
  ) : 16'd0))));

  assign stream_in_hs =
      ((stream_active_r && !is_output_dir) || start_input_stream)
      && stream_in_valid
      && stream_in_ready;
  ...
  assign gbp_stream_done =
      ((!is_output_dir || start_input_stream)
       && stream_in_hs
       && ((stream_in_beats_r + 1) == stream_target_beats))
      || ((is_output_dir || start_output_stream)
          && stream_out_hs
          && ((stream_out_beats_r + 1) == stream_target_beats));
